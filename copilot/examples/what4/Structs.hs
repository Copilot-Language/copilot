...
instance Struct Volts where
  ...
  -- needed to make the Copilot interpreter work, and we do not use the
  -- interpreter in this example. (See `examples/Structs.hs` for an example that
  -- does implement `updateField`.)
