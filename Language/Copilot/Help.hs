...
helpStr =
  ...
  , " > test STREAMS FILENAME RNDS (noOpts | [$ OPTS])"
  , "   Takes the same arguments as compile, with a RNDS argument telling it how" 
  , "   many periods to execute.  Executes the interpreter and the compiled C"
  , "   program for that program for the specified number of rounds, comparing"
  ...
  , "   external arrays is currently not supported.  When testing random"  
  , "   we delete the generating programs after testing them."
  , ""
  ...
  , ""
  , " > P.sequence_ $ P.replicate 100 (randomTest 10 baseOpts)"
  , "   Compare the compiler and interpreter on 100 random programs, each"
